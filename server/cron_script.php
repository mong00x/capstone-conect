...
// set database time to darwin time
$query_set_time_zone = "SET time_zone = '-6:00'";
connectDB();
$result_set_time_zone = mysqli_query($_SESSION['db'], $query_set_time_zone);    
closeDB();

// read cron.txt file if it exists
if (file_exists('cron.txt')) {
    $cron_file = fopen('cron.txt', 'r');
    $cron_file_contents = fread($cron_file, filesize('cron.txt'));
    fclose($cron_file);
    $cron_file_contents = explode('

', $cron_file_contents);
// if not exist create it
} else {
    $cron_file = fopen('cron.txt', 'w');
    $cron_file_contents = array();
}

// write current dtaatime in it
$cron_file = fopen('cron.txt', 'w');
fwrite($cron_file, date('Y-m-d H:i:s'));
fclose($cron_file);
...
//         // $LECmail->Password = 'BrainDrain';
         ...
         $LECmail->Subject = 'New project request from scheduler system';
        //  $LECmail->isHTML(true);
